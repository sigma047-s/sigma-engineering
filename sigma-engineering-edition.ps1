...
.DESCRIPTION
    ...
    every requirement each app declares (OS, CPU, RAM, disk, GPU, DirectX, .NET,
    VC++, Java, Python, license services, Windows features, admin rights),
    verifies online where possible (PassMark, latest published versions),
    and reports MEETS / PARTIALLY MEETS / DOES NOT MEET per app. Installer
    preserved (winget + manual vendor pages).
#>
[CmdletBinding()]
param(
    [string[]]$Disciplines = @(),
...
function Show-InstallableFromScan {
    ...
    $installable = @($ScanResults | Where-Object { $_.Verdict -in @('MEETS','MEETS (unverified)') -and -not $_.Installed })
    if ($installable.Count -eq 0) { Write-Host "  No installable apps from the scan." -ForegroundColor Yellow; return }
    for ($i=0; $i -lt $installable.Count; $i++) {
        $p = $installable[$i]; $tag = Get-InstallTag -Name $p.Product
        $col = switch ($tag) { 'winget' { 'Green' } 'manual' { 'Yellow' } default { 'DarkGray' } }
        Write-Host ("    {0,2}. {1,-32} [{2}]" -f ($i+1), $p.Product, $tag) -ForegroundColor $col
    }
    Write-Host "     0. Cancel"
    $sel = Read-Host "  Numbers to install (e.g. 1,3) or 'all'"
    if (-not $sel -or $sel -eq '0') { return }
    $picked = @()
    if ($sel -match '^all$') { $picked = $installable | ForEach-Object { $_.Product } }
    else {
        foreach ($tok in ($sel -split ',')) {
            $n = 0
            if ([int]::TryParse($tok.Trim(),[ref]$n) -and $n -ge 1 -and $n -le $installable.Count) { $picked += $installable[$n-1].Product }
        }
    }
...
$finalChoice = Read-Host "Press R to open report folder, I to install from scan results, Q to quit"
switch -Regex ($finalChoice) {
    '^[Rr]$' { Start-Process $exportPath }
    '^[Ii]$' { Show-InstallableFromScan -ScanResults $scanResults }
    default  { exit 0 }
}
